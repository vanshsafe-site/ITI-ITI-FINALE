...
function AdminIndex() {
  const [counts, setCounts] = useState({ users: 0, videos: 0, posts: 0, gallery: 0 });

  useEffect(() => {
    (async () => {
      const [u, v, b, g] = await Promise.all([
        supabase.rpc("admin_list_users"),
        supabase.from("videos").select("id", { count: "exact", head: true }),
        supabase.from("blog_posts").select("id", { count: "exact", head: true }),
        supabase.from("gallery_items").select("id", { count: "exact", head: true }),
      ]);
      setCounts({ users: (u.data as any[])?.length || 0, videos: v.count || 0, posts: b.count || 0, gallery: g.count || 0 });
    })();
  }, []);
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20 }}>
      ...
      {stat("Gallery items", counts.gallery, "/admin/gallery")}
