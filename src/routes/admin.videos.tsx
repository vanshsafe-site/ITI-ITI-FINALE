...
function empty(): Video {
  return { id: "", title: "", description: "", youtube_id: "", thumbnail_url: "", sort_order: 0, is_free: false };
}
        ...
        {videos.map((v) => (
          <div key={v.id} className="card" style={{ padding: 0, overflow: "hidden" }}>
            ...
            <div style={{ padding: 16 }}>
              ...
              <div style={{ marginTop: 10, display: "inline-flex", alignItems: "center", gap: 8, fontSize: "0.78rem", color: v.is_free ? "#1a6d3b" : "var(--muted)" }}>
                <span style={{ width: 8, height: 8, borderRadius: 999, background: v.is_free ? "#1a6d3b" : "#9a9a9a" }} />
                {v.is_free ? "Free preview available" : "Members only"}
              </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setEditing(null)}>
          <div className="card" style={{ maxWidth: 500, width: "100%" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--forest)", fontWeight: 600, fontSize: "0.9rem" }}>
                <input type="checkbox" checked={editing.is_free} onChange={(e) => setEditing({ ...editing, is_free: e.target.checked })} />
                Visible to logged-in users before activation (free preview)
              </label>
              <div>
