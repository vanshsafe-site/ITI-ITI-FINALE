...
export function ProtectedVideoPlayer({ video, onClose }: { video: Video; onClose: () => void }) {
  ...
  return (
    ...
    >
      <button
        onClick={onClose}
        style={{ position: "absolute", top: 20, right: 20, background: "var(--dusk)", color: "var(--forest)", border: "1px solid var(--border)", borderRadius: 40, padding: "10px 20px", cursor: "pointer", fontWeight: 500 }}
      >
        ✕ Close
      </button>

      <div style={{ width: "100%", maxWidth: 1000 }}>
        <div style={{ color: "var(--forest)", fontFamily: "'Cormorant Garamond', serif", fontSize: "1.6rem", marginBottom: 16 }}>
          {video.title}
        </div>
        <div
          style={{
        ...
        {video.description && (
          <p style={{ color: "rgba(232,234,246,0.7)", marginTop: 16, lineHeight: 1.7 }}>{video.description}</p>
        )}
        <p style={{ color: "rgba(232,234,246,0.4)", marginTop: 12, fontSize: "0.8rem" }}>
